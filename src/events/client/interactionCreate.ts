...
export default class implements Event {
	...
	public async execute(interaction: Interaction) {
		...
		if (interaction.inCachedGuild()) {
			...
			// TODO: add setting to prisma - vetVerificationChannelId
			const verificationChannelId = '924145472728363109';
			if (interaction.channelId === verificationChannelId) ephemeral = true;

			await interaction.deferReply({ ephemeral }).catch(() => undefined);
			...
			if (raid) {
				...
				if (interaction.channelId === raid.channelId) {
					if (interaction.customId === 'portal') {
						await interaction.editReply({
							content: 'You successfully joined the raid. Please wait for further instructions.',
						});
					}
				}
			}
